...
class ProcConnRecipe < OtherRecipe
  ...
  self.connection_lambda = proc do |conn|
    conn.url_prefix = "http://proc-sushi.com/api/v1"
    next conn
  end
end
...
class LambdaConnRecipe < OtherRecipe
  ...
  self.connection_lambda = lambda { |conn|
    conn.url_prefix = "http://lambda-sushi.com/api/v1"
    conn
  }
end
...
class MethConnRecipe < OtherRecipe
  ...
  def self.connection_finder(conn)
    conn.url_prefix = "http://meth-sushi.com/api/v1"
    conn
  end

  self.connection_lambda = method(:connection_finder)
end
...
class SymbolConnRecipe < OtherRecipe
  ...
  def self.connection_finder(conn)
    conn.url_prefix = "http://symbol-sushi.com/api/v1"
    conn
  end
end
